...
static int _recursive_rmdir_onedev(char *dirname, dev_t pdev, const char *exclude, int level)
{
...
__attribute__((constructor)) static void gnutls_lxc_init(void)
{
    gnutls_global_init();
}
...
// 这个函数 on_path 的作用是查找给定命令 cmd 是否存在于系统的 PATH 环境变量指定的目录中，并返回该命令的完整路径。如果指定了 rootfs，则在该根文件系统下查找命令。
